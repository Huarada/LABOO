...
void Hospedeiro::adicionarChat(int porta, int enderecoDestino, int portaDestino) {
    if (getChat(porta) != NULL) throw new logic_error("Ja tem chat na porta");

    Chat *chatHosp = new Chat(this, porta, enderecoDestino, portaDestino);
    chats->push_back(chatHosp);
    quantidadeDeChats++;
}
